...
export default function AdminMapManager({ initialTransitId = null }) {
  ...
  const updateTransitStopsAndGeometry = async (newStops) => {
    ...
    if (newStops.length >= 2) {
      ...
      if (selectedId) {
        ...
        const currentFeat = customTransitLines.find(t => t.properties.id === selectedId);
        const feat = featToSave || (currentFeat ? {
          ...currentFeat,
          properties: { ...currentFeat.properties, paradas: newStops },
          geometry: geom
        } : null);
        ...
        if (feat) {
          ...
          }).catch(err => console.warn('[TRANSIT] Error al persistir línea:', err));
        }
      }
    } else {
      setTransitRouteGeometry(null);
  ...
  const handleColorChangeTransit = (hex) => {
    ...
    if (selectedId) {
      ...
      const feat = featToSave || (currentFeat ? {
        ...currentFeat,
        properties: { ...currentFeat.properties, color: hex }
      } : null);
  ...
  const handleUpdateStopName = (index, newName) => {
    ...
    if (selectedId) {
      ...
      const feat = featToSave || (currentFeat ? {
        ...currentFeat,
        properties: { ...currentFeat.properties, paradas: nextStops }
      } : null);
  ...
  const promptDeleteMeetingPoint = (id) => {
    const p = customMeetingPoints.find(item => String(item.id) === String(id));
    setConfirmModal({
      isOpen: true,
      ...
      isDanger: true,
      onConfirm: () => {
        const next = customMeetingPoints.filter(item => String(item.id) !== String(id));
        setCustomMeetingPoints(next);
        saveLiveMeetingPoints(next);
        handleResetSelection();
        setConfirmModal(prev => ({ ...prev, isOpen: false }));
  ...
  const promptDeletePlace = (id) => {
    const p = places.find(item => String(item.id) === String(id));
    setConfirmModal({
      isOpen: true,
  ...
  return (
    <div className="space-y-4 sm:space-y-5 text-left w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-start">
        ...
        <div className="lg:col-span-4 bg-white/95 backdrop-blur-xl rounded-3xl p-4 sm:p-5 border border-sky-100 shadow-sm space-y-4 h-[480px] lg:h-[520px] overflow-y-auto">
            ...
            <div className="space-y-3.5">
              ...
              <div>
                ...
                <select
                  value={selectedId || ''}
                  onChange={(e) => {
                    const p = customMeetingPoints.find(item => String(item.id) === String(e.target.value));
            ...
            <form onSubmit={handleSavePlace} className="space-y-3.5">
              ...
              <div>
                ...
                <select
                  value={selectedId || ''}
                  onChange={(e) => {
                    const p = places.find(item => String(item.id) === String(e.target.value));
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      setPlaceForm({
                        ...
                        type: selectedCat ? selectedCat.type : placeForm.type
                      });
                    }}
                    className={inputClass}
                  >
        ...
        {/* ============================================================
           ...
           ============================================================ */}
        <div className="lg:col-span-8 h-[480px] lg:h-[520px] rounded-3xl overflow-hidden border border-sky-100 shadow-md relative">
            ...
            {editorTab === 'meetingPoint' && (
              <Marker
                ...
                onDragEnd={(e) => {
                  ...
                  setMeetingForm(prev => ({ ...prev, lng: newLng, lat: newLat }));
                  if (selectedId) {
                    setCustomMeetingPoints(prev => {
                      const next = prev.map(p => (p.id === selectedId ? { ...p, lng: newLng, lat: newLat } : p));
                      saveLiveMeetingPoints(next);
                      return next;
                    });
                  }
            ...
            {editorTab === 'place' && (
              <Marker
                ...
                onDragEnd={(e) => {
                  const newLng = parseFloat(e.lngLat.lng.toFixed(6));
                  const newLat = parseFloat(e.lngLat.lat.toFixed(6));
                  setPlaceForm(prev => ({ ...prev, lng: newLng, lat: newLat }));
                  if (selectedId) {
                    updatePlace(selectedId, { ...placeForm, lng: newLng, lat: newLat });
                  }
                }}
              >
            ...
            {editorTab === 'light' && (
              <Marker
                ...
                onDragEnd={(e) => {
                  const newLng = parseFloat(e.lngLat.lng.toFixed(6));
                  const newLat = parseFloat(e.lngLat.lat.toFixed(6));
                  setLightForm(prev => ({ ...prev, lng: newLng, lat: newLat }));
                  if (selectedId) {
                    setCustomLights(prev => {
                      const next = prev.map(l => (l.properties.id === selectedId ? {
                        ...l,
