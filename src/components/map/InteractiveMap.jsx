...
export default function InteractiveMap({
  ...
  // Sincronizar líneas de transporte vial desde el backend SQLite
  useEffect(() => {
    fetch(`${API_BASE_URL}/transit`)
      .then(r => (r.ok ? r.json() : null))
      .then(data => {
        if (data && data.features && data.features.length > 0) {
          setLiveTransitLines(data);
          saveLiveTransitLines(data.features);
        }
      })
      .catch(err => console.warn('[InteractiveMap] Error cargando líneas de backend:', err));
  }, []);
  ...
  return (
    <div className="w-full space-y-3 sm:space-y-4">
      ...
      >
        ...
        {/* Marcador de destino seleccionado si no está en la lista filtrada */}
        {selectedPlace && !filteredPlaces.some(p => String(p.id) === String(selectedPlace.id)) && (
          <Marker
        ...
        {(userGpsLocation || userLocation) && (
          <Marker
            longitude={(userGpsLocation || userLocation)[0]}
            latitude={(userGpsLocation || userLocation)[1]}
            anchor="center"
          >
            <div className="relative flex items-center justify-center">
              ...
              <div className="w-6 h-6 bg-sky-600 rounded-full border-2 border-white shadow-md relative z-10 flex items-center justify-center text-white">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {selectedPlace.rating && (
                  <span className="text-xs font-bold text-amber-500 flex items-center gap-1">
                    <LucideIcons.Star size={12} className="fill-amber-400 text-amber-400" /> {selectedPlace.rating}
                  </span>
                )}
              </div>
