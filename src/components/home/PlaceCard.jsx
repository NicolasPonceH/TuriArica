...
export default function PlaceCard({ place, onAudioClick, onRouteClick, onMoreClick }) {
  ...
  const eventsContext = useEvents ? useEvents() : null;
  const activeEvents = eventsContext?.activeEvents || [];
  const activePromo = activeEvents.find(e => e.isActive && (String(e.placeId) === String(place.id) || (e.placeName && e.placeName.toLowerCase() === place.name.toLowerCase())));
  ...
  return (
    ...
    >
      ...
      >
        {/* Photo if available and not broken */}
        {!imgError && place.photos && place.photos.length > 0 ? (
          <img
          ...
          <div 
            className="absolute inset-0 flex flex-col items-center justify-center p-4 text-white text-center"
            style={{ background: `linear-gradient(135deg, ${place.color || '#0284c7'}, #0f172a)` }}
          >
            <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center mb-1.5 shadow-inner">
              <IconComponent size={24} className="text-white drop-shadow-md" />
            </div>
            <span className="text-[11px] font-black tracking-wider uppercase opacity-85">{place.category}</span>
          </div>
        )}
        ...
        {/* Promo / Discount Badge for local businesses */}
        {activePromo && (
          <span className="absolute bottom-3 left-3 px-2.5 py-1 bg-gradient-to-r from-rose-500 to-amber-500 text-white text-[11px] font-black rounded-lg shadow-md flex items-center gap-1 z-10">
            <LucideIcons.Tag size={12} />
            <span>{activePromo.discountBadge || activePromo.title}</span>
          </span>
        )}
