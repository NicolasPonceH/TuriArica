...
export function PlacesProvider({ children }) {
  ...
  const fetchPlaces = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const activePlaces = data.filter(p => !deletedIds.map(String).includes(String(p.id)));
    ...
    const stored = getStoredPlaces().filter(p => !deletedIds.map(String).includes(String(p.id)));
    const activeDefaults = defaultPlaces.filter(p => !deletedIds.map(String).includes(String(p.id)));
  ...
  const updatePlace = useCallback(async (id, data) => {
    ...
    try {
      ...
      if (res.ok) {
        const resData = await res.json();
        setPlaces(prev => prev.map(p => (String(p.id) === String(id) ? resData.place : p)));
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent(GEODATA_UPDATED_EVENT));
        }
        return true;
    ...
    // Local fallback
    setPlaces(prev => prev.map(p => (String(p.id) === String(id) ? { ...p, ...payload } : p)));
    setAdminPlaces(prev => {
      const updated = prev.map(p => (String(p.id) === String(id) ? { ...p, ...payload } : p));
      setStoredPlaces(updated);
      return updated;
    });
    if (typeof window !== 'undefined') {
